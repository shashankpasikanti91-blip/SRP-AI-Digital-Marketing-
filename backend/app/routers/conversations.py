...
class MessageCreate(BaseModel):
    content: str = Field(..., min_length=1)
    sender_role: SenderRole = SenderRole.CONTACT
...
async def list_messages(
    ...
    query = select(ConversationMessage).where(ConversationMessage.conversation_id == conv_id)
    total = (await db.execute(select(func.count()).select_from(query.subquery()))).scalar_one()
    query = query.order_by(ConversationMessage.sent_at.asc()).offset((page - 1) * page_size).limit(page_size)
    items = list((await db.execute(query)).scalars().all())
    return {"items": items, "total": total}
...
async def add_message(conv_id: uuid.UUID, payload: MessageCreate, tenant: CurrentTenant, db: DB):
    ...
    msg = ConversationMessage(
        ...
        sender_name=payload.sender_name or conv.contact_name,
...
@router.post("/{conv_id}/ai-reply")
async def generate_ai_reply(conv_id: uuid.UUID, payload: AIReplyRequest, tenant: CurrentTenant, db: DB):
    """Use Conversation Agent to generate a smart AI reply."""
    conv_res = await db.execute(
        select(Conversation).where(Conversation.id == conv_id, Conversation.tenant_id == tenant.id)
    )
    conv = conv_res.scalar_one_or_none()
    ...
    # Build history
    history = [{"role": (m.sender_role.value if hasattr(m.sender_role, 'value') else str(m.sender_role)), "content": m.content} for m in messages]
...
async def update_status(conv_id: uuid.UUID, payload: StatusUpdate, tenant: CurrentTenant, db: DB):
    ...
    if not conv:
        raise HTTPException(status_code=404, detail="Conversation not found")
    conv.status = payload.status
    if payload.status == ConversationStatus.READ:
        conv.unread_count = 0
    await db.flush()
    await db.refresh(conv)
    return conv
