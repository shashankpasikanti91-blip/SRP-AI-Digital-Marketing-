"""AI Service — centralized AI capabilities using model_router for all calls."""
...
# ── AI helper (routed via model_router) ─────────────────────────────────

async def _ai_call(
    system: str,
    user: str,
    output_model: type[BaseModel],
    bucket: FeatureBucket = FeatureBucket.text_marketing,
) -> tuple[Any, int]:
    """Call AI via model_router with JSON mode; parse into given Pydantic model."""
    ...
    response = await client.chat.completions.create(
        ...
        max_tokens=getattr(settings, "AI_MAX_TOKENS", 2500),
        temperature=getattr(settings, "AI_TEMPERATURE", 0.75),
    )
...
async def _ai_text(
    system: str,
    prompt: str,
    bucket: FeatureBucket = FeatureBucket.text_basic,
) -> str:
    """Plain text AI call via model_router."""
    router = get_model_router()
    client, model_id = router.resolve(bucket)
    response = await client.chat.completions.create(
        model=model_id,
...
class AIService:
    ...
    async def generate_social_post(payload: GeneratePostRequest) -> GeneratePostResponse:
        prompt = (
            f"Platform: {payload.platform.value if hasattr(payload.platform, 'value') else str(payload.platform)}\n"
    ...
    async def chat(system: str, messages: list) -> str:
        try:
            from app.services.model_router import FeatureBucket, get_model_router
            router = get_model_router()
            client, model_id = router.resolve(FeatureBucket.chatbot)
